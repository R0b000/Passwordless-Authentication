...
namespace Auth.UI.Components.Pages.Shared.Passkey
{
    public partial class PasskeySetup : ComponentBase
    {
        ...
        protected async Task StartRegistrationAsync()
        {
            ...
            var optionsResult = await AuthManager.RequestAttestationOptionsAsync(new Fido2AttestationOptionsRequest
            {
                UserId = UserId,
                Username = Username,
                Origin = origin
            ...
            try
            {
                ...
                if (verifyResult.Succeeded && verifyResult.Data?.Success == true)
                {
