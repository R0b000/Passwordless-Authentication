...
namespace Auth.UI.Manager.Implementation.Auth
{
    public class SecurityManager : ISecurityManager
    {
        ...
        public async Task<IResponse<SecuritySettingsResponse>> GetSecurityAsync()
        {
            try
            {
                var result = await _httpService.GetAsync<SecuritySettingsResponse>(SecurityRoute.Settings);
                if (result.Succeeded && result.Data is not null)
                {
                    return Response<SecuritySettingsResponse>.Success(result.Data, "Security settings retrieved");
                }
                else
                {
        ...
        public async Task<IResponse<SecuritySettingsResponse>> UpdateSecurityAsync(SecuritySettingsRequest settings)
        {
            try
            {
                 var result = await _httpService.PostAsJsonAsync<SecuritySettingsResponse>(SecurityRoute.Settings, settings);
                if (result.Succeeded && result.Data is not null)
                {
                    return Response<SecuritySettingsResponse>.Success(result.Data, "Security settings updated");
                }
                else
                {
            ...
            {
                return Response<SecuritySettingsResponse>.Fail($"An error occurred while updating the security settings: {ex.Message}");
            }
        }
        ...
        public async Task<IResponse<SecuritySettingsResponse>> EnableTwoFactorAsync()
        {
            try
            {
                var result = await _httpService.PostAsJsonAsync<SecuritySettingsResponse>(SecurityRoute.Enable2FA, null!);
                if (result.Succeeded && result.Data is not null)
                {
                    return Response<SecuritySettingsResponse>.Success(result.Data, "Two-factor authentication enabled");
                }
                else
                {
        ...
        public async Task<IResponse<SecuritySettingsResponse>> DisableTwoFactorAsync()
        {
            try
            {
                var result = await _httpService.PostAsJsonAsync<SecuritySettingsResponse>(SecurityRoute.Disable2FA, null!);
                if (result.Succeeded && result.Data is not null)
                {
                    return Response<SecuritySettingsResponse>.Success(result.Data, "Two-factor authentication disabled");
                }
                else
                {
            ...
            {
                return Response<SecuritySettingsResponse>.Fail($"An error occurred while disabling two-factor authentication: {ex.Message}");
            }
        }
