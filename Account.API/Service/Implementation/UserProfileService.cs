...
namespace Account.API.Service.Implementation
{
    public class UserProfileService : IUserProfileService
    {
        ...
        public async Task<IResponse<UserProfileResponse?>> GetProfileAsync(int userId)
        {
            var userResult = await _userRepository.QuerySingleAsync<User>(
                ...
                new
                {
                    AuthType = DbConstants.AuthTypes.Login,
                    UserId = userId
                }
            );
            ...
            if (userResult.Succeeded && userResult.Data != null)
            {
                var profileResult = await _userRepository.QuerySingleAsync<UserProfileResponse>(
                    ...
                    new
                    {
                        AuthType = DbConstants.AuthTypes.GetProfile,
                        UserId = userId
                    }
                );
                if (profileResult.Succeeded && profileResult.Data != null)
                    return Response<UserProfileResponse?>.Success(profileResult.Data);
                return Response<UserProfileResponse?>.Success(new UserProfileResponse { UserId = userResult.Data.Id, Username = userResult.Data.Username ?? string.Empty, Email = userResult.Data.Email ?? string.Empty, DateJoined = userResult.Data.CreatedAt, AccountStatus = "active" });
            }
